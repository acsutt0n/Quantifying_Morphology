...
def show_quad_path(initDiam, pathLength, param_index, N=100):
  """
  Fit, show and return the quadratic taper starting with diameter of
  initDiam to a distance of pathLength using ldend[param_index] and 
  P[param_index]; _param_index_ should be between 0 and 56. N = # samples
  """
  def solve(params, x):
    return params[0]*x**2 + params[1]*x + params[2]
  P, ldend = load_params() 
  
  xs = np.linspace(0., ldend[param_index], N)
  ys = [solve(P[param_index], i) for i in xs]
  # Now plot?
