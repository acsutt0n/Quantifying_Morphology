# fixSegments.py -- loads in hoc file and fixes bad radii and segments
# usage: python fixSegments.py hocFile space(op) newFile(op)
#        space = is there a space between the points in pt3dadd
#
...
class HocGeometry:
  def __init__(self, fileName, space=False, newFile=False):
...
# space is 2nd argument, newfile (opt) is last
##################################
if __name__ == '__main__':
  ...
  if len(arguments) > 3:
    ...
    space, newFile = False, False
  
  geometry = HocGeometry(hocFile, space, newFile)
