...
class HocGeometry(Geometry):
  ...
  def getSegLengths(self):
    ...
    attrs = vars(self.segments)
    print(', '.join('%s: %s' % item for item in attrs.items()))
   # segLengths = [self.segments.length for segments in 
   #               range(len(self.segments))]
                  
   # for segs in range(len(self.segments)):

    
    #return len(segLengths)
...
def demoRead(geoFile):
  ...
  #coords = [pDF.tipCoords(tip,pos)
  #          for tip, pos in zip(tips, tipPositions)]
  print('num of tips: %i' %len(tips))
  print('num of tipPositions: %i' %len(tipPositions))
  
  
  numsegs = geometry.getSegLengths()
  print('number of segments: %i' %numsegs)
  
  # get seg lengths and centroids
  #lens = [se
  
#  # pCoords = np.array(coords)
#  # print(pCoords)
#  # print('%d carrier points found', % len(coords))
#  
# with open('results.txt', 'w') as results_file:
#    for pt in range(len(coords)):
#      pString = str(coords[pt])
#      pString = pString.strip().split('(')
#      pString = pString[1].split(')')
#      pString = pString[0].split(',')
#      pString = ' '.join(pString)
#      results_file.write(str(pString))
#      results_file.write('\n')
#  
