...
def returnTorques(seg1list, seg2list, planCoords):
  # pass this some stuff, it will return stuff about the torque
  ### not used in current version
  lista, listb = [], []
  
  def returnFirstConnection(segg, seg1list, seg2list, planCoords):
    # this stupid function keeps going until it finds a connected segment
    # that is also a key of planCoords (meaning a bifurcation segment)
    segpartner = None
    flip = 1 # tells the current location of segpartner (which list to check)
    tempseg = segg
    prevsegs = [segg]
    
    while segpartner not in planCoords.keys():
      if flip == 1: # if at the 1th end
        count = 0   # start count
        for s in seg2list: # for all the segments in the seglist
          if s == prevsegs[-1]: # if s is the current segment
            if seg1list[count] not in prevsegs: # and not already added
              segpartner = s # set new segpartner
              flip = (flip+1)%2 # which is now a 0th end
              break # break the loop
          count = count + 1 # else, increase the count
      elif flip == 0:
        count = 0
        for s in seg1list:
          if s == prevsegs[-1]:
            if seg2list[count] not in prevsegs:
              segpartner = s
              flip = (flip+1)%2
              break
          count = count + 1
      prevsegs.append(segpartner)
    
    # should return a valid segpartner now; this is the first connected
    # segment that has a plane associated with it
    return segpartner
      
      
  
  # loop through connections (from 1 to 0) and map torques
  torques = []
  # loop through plane keys; should have >= len(planCoords.keys())-1 total
  for s in planCoords.keys():
    torques.append(angleBetween(s,returnFirstConnection(s,seg1list,seg2list,planCoords),
                                planCoords))
...
def demoRead(geoFile):
  ...
  planCoordskeys = []
  for s in geometry.segments: # loop through segments to find plane-neighbors
  ...
  """
  torques = returnTorques(Seg1s, Seg2s, planCoords)
  torques = [torques[i] for i in range(len(torques)) if not
             np.isnan(torques[i])]
  print(torques)
  print(len(torques), len(planCoords))
  """
