# convert to SWC file for L-measure 
#######
# usage: $ python convertHocSWC.py hocFileName swcFileName root/soma# axon#
#   integers should put input directly into the command line for root
#   for axon sections
# assumes root is soma (label 1) and axon segment gets a label of 2

def convertHoc(hocFile, rootSeg, axonSeg):
  import numpy as np
  ...
  def writeSWCline(current_section, iden, xmean, ymean, zmean, radmean):
    # find parent
    if current_section == int(rootSeg): # use manually identified root section
      ind = -2
    else:
      try:
...
if __name__ == '__main__':
  ...
  rootSeg = sys.argv[3]
  axonSeg = sys.argv[4]
  convertHoc(hocFile, rootSeg, axonSeg)
